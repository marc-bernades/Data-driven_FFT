...
class FFT_process:
    def __init__(self, save_dir, cases, dt, delta, L_x, L_z,
        ...
        self.output_fft_space, self.kx = self.preprocess_fft_space()
    ...
    def preprocess_snapshots(self):
        ...
        z_plane   = int(n_z/2)
        var       = 1
        snapshots = snapshots[var,:,z_plane,:,:]
        return snapshots
    ...
    def preprocess_fft_space(self):
        ...
        d_dir = self.save_dir
        #np.save( f"{d_dir}/output_fft_space.npy", output_fft_space)
        #np.save( f"{d_dir}/kx.npy", kx)
    ...
    def preprocess_fft_time(self):
        ...
        d_dir = self.save_dir
        np.save( f"{d_dir}/output_fft_time_XY.npy", output_fft_time)
    ...
    def obtain_spectra_target(self,c_plus_target, wall_str):
        ...
        c_ammend   = self.omega[(np.abs(c_map[:idx_omega_plus,idx_kx_ref] - c_target)).argmin()]*kx_inv[idx_kx_ref]
        #print(self.omega[(np.abs(c_map[:,idx_kx_ref] - c_target)).argmin()]*kx_inv[idx_kx_ref])
        c_idx      = c_map == c_ammend
        #print(c_ammend)
        #print(c_ammend.shape)
        #print("omega")
        #print(self.omega)
        #print(np.max(self.omega))
        #print("kx")
        #print(self.kx)
        #print(np.max(self.kx))
        #print("c_idx")
        #print(np.argwhere(c_idx))
    ...
    def plot_c_target_XY(self,output_target,c_plus_target,wall_str,n_snapshot):
        ...
        cs = plt.tricontourf( x_data_norm, y_data_norm, u_data_norm, cmap = "bwr", norm = my_norm, levels = np.arange( u_min, u_max + 1e-1, 1.0e-3 ) )

        # Colorbar
        cbar = plt.colorbar( cs, shrink = 0.15, pad = 0.02, ticks = [0,0.5*u_max,u_max] )
        cbar.ax.tick_params( labelsize = 9 ) 
        plt.text( 12.9, 2.05, r'$u^{+}$', fontsize = 9 )
        plt.clim( -u_max, u_max )
        ...
        plt.savefig(f'figures/Cond_phase_speed_XY/XY_u_c_plus_target_{label_save}_{wall_str}_{n_snapshot}.png', format = 'png', bbox_inches = 'tight', dpi=600 )
